(function(Scratch) {
    ...
    class divEffect {
        ...
        static *runHandler(body, handlers, val = null) {
            ...
            while(!yielded.done) {
                if(yielded.value instanceof divEffect && handlers[yielded.value.eff]) {
                    return yield* handlers[yielded.value.eff](yielded.value.data, function*(val) {
                        return yield* divEffect.runHandler(body, handlers, val)
                    }, function*(body) {
                        return yield* divEffect.runHandler(body, handlers)
                    });
                }
                yield yielded.value
                yielded = body.next()
    ...
    const divAlgEffects = {
        ...
        continuation(divEffResume) {
            ...
            return new this.contType(function*(arg) {return yield* divEffResume(arg)})
        },
        ...
        getInfo = () => ({
            ...
            blocks: [
                ...
                },
                {
                    ...
                    disableMonitor: true,
                    blockType: BlockType.COMMAND,
                    notchAccepts: NotchShape.HEXAGON,
                    branchCount: 1,
                    arguments: {
                        EFF: {type: ArgumentType.STRING, defaultValue: "effect"},
                        DATA: {fillIn: 'effData'},
        ...
        getCompileInfo = () => ({
            ...
            js: {
                ...
                effHandle(node, compiler, imports) {
                    ...
                    const substack1 = descendSubstack(node.SUBSTACK1, compiler, new imports.Frame(true));
                    const effCases = node.effCases.map(({eff, handler}) =>
                     /*js*/`${compiler.descendInput(eff).asString()}: function*(divEffData, divEffResume, divEffRehandle) {\n`
                          +`    do {${descendSubstack(handler, compiler, new imports.Frame(true, "divAlgEffects.effHandlerCase"))}} while(false);\n`
                          +`},`
                    ).join("");
                    compiler.source +=
                    /*js*/`const ${body} = (function*() {do {${substack1}} while(false)})();\n`
                   +/*js*/`const ${handlers} = {\n${effCases}\n};\n`
                   +/*js*/`yield* vm.divAlgEffects.Effect.runHandler(${body}, ${handlers});`
                },
                ...
                effRecurseHandler(node, compiler, imports) {
                    ...
                    const substack = descendSubstack(node.SUBSTACK, compiler, new imports.Frame(false));
                    compiler.source +=
                    /*js*/`return yield* divEffRehandle((function*() {do {${substack}} while(false)})());`
                },
                effResume(node, compiler, imports) {
                    if(!compiler.frames.some(f => f.parent === "divAlgEffects.effHandlerCase")) return;
                    compiler.source +=
                    /*js*/`return yield* divEffResume(${compiler.descendInput(node.DATA).asUnknown()});`
                },
